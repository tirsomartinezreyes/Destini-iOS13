...
struct App {
    ...
    mutating func changeStory(_ destination:Int){
        let tmp = self.stories[destination] as Story?
        if(tmp != nil){
            self.currentStory = self.stories[destination]
        }
    }
